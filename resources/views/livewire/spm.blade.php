<div class="fixed w-full max-w-xl p-6 overflow-hidden transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-lg top-1/2 left-1/2 dark:bg-gray-800">
    ...
    <form wire:submit.prevent="save">
        ...
        <!-- Botão de alternância para upload de arquivo ou URL -->
        <div class="flex items-center mb-4">
            <label for="uploadOption" class="mr-4 text-sm font-semibold text-gray-700 dark:text-gray-300">Escolha uma opção:</label>
            <div class="relative">
                <!-- Botão de switch -->
                <button type="button" x-data="{ fileUpload: true }" @click="fileUpload = !fileUpload" :class="{ 'bg-green-500' : fileUpload, 'bg-gray-300': !fileUpload }" class="flex items-center w-12 h-6 transition-colors duration-200 ease-in-out rounded-full focus:outline-none">
        ...
        <div class="mb-4" x-show="!fileUpload">
            <label for="imagem_url" class="block text-sm font-semibold text-gray-700 dark:text-gray-300">URL da Imagem:</label>
            ...
            @error('imagem_url') <span class="text-sm text-red-500">{{ $message }}</span> @enderror
        </div>
